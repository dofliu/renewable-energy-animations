...
usage: python build.py EPISODE.js --out OUT.html

The episode file declares the kits it needs on its first lines, e.g.
    // KITS: marine
    // KITS: land
    // KITS: marine, offshore-seq, offshore-farm
Order in the output: core → i18n → kits → episode → EPISODE.i18n.js (translations, next to the episode file) → engine.
"""
...
ap=argparse.ArgumentParser();ap.add_argument('episode');ap.add_argument('--out',required=True);a=ap.parse_args()
...
js='\n'.join([rd('core.js'),rd('i18n.js')]+[rd(KIT_FILES[k]) for k in ORDER if k in kits]+[src,tsrc,rd('engine.js')])
...
html=rd('template.html').replace('__CSS__',rd('style.css')).replace('__MUXER__',rd('mp4-muxer.js')).replace('__JS__',js).replace('__TITLE__',title)
os.makedirs(os.path.dirname(os.path.abspath(a.out)),exist_ok=True)
open(a.out,'w',encoding='utf-8').write(html)
print(f'built {a.out}  ({len(html)//1024} KB, kits: {sorted(kits) or "none"})')
